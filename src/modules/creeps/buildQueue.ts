...
const sortBuildQueueByPriority = () => {
  for (let i = 1; i < Memory.buildQueue.length; i++) {
    ...
    for (let j = Memory.buildQueue.length - 1; j >= i; j--) {
      if (
        creepConfig[Memory.buildQueue[j].type].priority(
          Game.getObjectById(Memory.buildQueue[j].spawnId).room.name
        ) >
        creepConfig[Memory.buildQueue[j - 1].type].priority(
          Game.getObjectById(Memory.buildQueue[j - 1].spawnId).room.name
        )
      ) {
        haveOrder = false;
  ...
  for (let i = 0; i < Memory.buildQueue.length; i++) {
    ...
    if (!hashMap.has(message.spawnId)) {
      ...
      const result = buildFactory.buildCreep(
        ...
        creepConfig[message.type].priority(
          Game.getObjectById(message.spawnId).room.name
        ) > 100
          ? false
          : message.mustUseAllEnergy
      );
      if (result == OK) {
...
const getCreepNumber = () => {
  if (Memory.creepNumEachRoomEachType == null)
    Memory.creepNumEachRoomEachType = {};
  ...
  Object.getOwnPropertyNames(Game.creeps).forEach((creepName) => {
    ...
    if (Memory.creepNumEachRoomEachType[creep.room.name] == null) {
      ...
      Memory.creepNumEachRoomEachType[creep.room.name][creep.memory.role]++;
  });
};
...
const checkCreepNumberAndBuild = () => {
  ...
  for (const roomName in Game.rooms) {
    ...
    for (const role in Memory.creepNumEachRoomEachType[room.name]) {
      if (
        Memory.creepNumEachRoomEachType[room.name][role] +
          Memory.swaningCreepNumEachRoomEachType[room.name][role] <
